...
if (isset($_POST['submit'])) {
	...
	$query	=	"SELECT * from akun , data_pendaftaran WHERE email='$email' 
				AND data_pendaftaran.id_akun = akun.id_akun";
	...
	if ($exec) {
		...
		if (mysqli_num_rows($exec) > 0) {
			...
			if (mysqli_num_rows($exec2) > 0) {
				while ($rows2 = mysqli_fetch_array($exec2)) {
					
					if (password_verify($password,$rows2['password'])) {
						$_SESSION['role_user'] 	= $rows2['role_user'];
						$_SESSION['auth']		= $rows2['id_akun'];
